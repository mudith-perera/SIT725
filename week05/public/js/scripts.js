...
const submitForm = () => {
  const title = $('#first_name').val() || $('#projectTitle').val() || 'Untitled';
  const author = $('#last_name').val() || $('#projectAuthor').val() || 'Anonymous';
  const link = $('#projectLink').val() || '#';
  const desciption = $('#projectDesc').val() || $('#projectDescription').val() || '';
  ...
  $.ajax({
    ...
    success: function(response) {
      if (response.statusCode === 201) {
        $('#modal1').modal('close');
        // clear form fields
        $('#first_name, #last_name, #projectTitle, #projectAuthor, #projectLink, #projectDesc, #projectImage').val('');
        getProjects();
      } else {
